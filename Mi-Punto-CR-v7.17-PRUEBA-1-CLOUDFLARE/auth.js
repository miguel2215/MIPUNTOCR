...
window.lockApp = () => {
  if (role === "employee") return window.employeeSessionLock();
...
window.savePinLock = async () => {
  ...
  state.settings.ownerPin = pin; state.settings.pinEnabled = true; await put("settings", state.settings); lastActivityAt = Date.now(); closeModal(); toast("Bloqueo con PIN activado."); render();
};
...
window.employeeSessionLock = async () => {
  try { await cloudLogoutOwner(); } catch (_) {}
  state.settings.sessionActive = false;
  state.settings.employeeId = "";
  state.settings.employeeCode = "";
  await put("settings", state.settings);
