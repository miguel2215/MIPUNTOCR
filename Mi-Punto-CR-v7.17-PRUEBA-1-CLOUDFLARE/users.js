...
async function renderUsers() {
  if (role !== "owner" || state.settings.guestMode) {
    screen = "home";
    toast("Los empleados se administran desde la cuenta del Dueño.");
    return renderHome();
  }
  $("#app").innerHTML = shell(`<section class="screen-title"><h2>Empleados</h2><p>Crea accesos simples con código CR.</p></section><div class="panel"><div class="row-head"><div><strong>Equipo</strong><div class="muted">Cada empleado recibe un código como CR1000.</div></div><button class="btn primary" onclick="openCreateEmployee()">Crear empleado</button></div><div id="employeeList" style="margin-top:14px"><div class="empty">Cargando empleados…</div></div></div>`, "users");
  try {
