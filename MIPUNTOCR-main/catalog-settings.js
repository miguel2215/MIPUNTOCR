...
function renderAdminSyncPanel() {
  ...
  let title = "Nube no vinculada";
  let detail = "Inicia sesión con una cuenta real para sincronizar los datos del negocio.";
  if (linked && !online) {
  ...
  return `<div class="panel" style="margin-top:14px">
    <div class="row-head">
      ...
      ${linked ? `<button class="btn ${pending ? "primary" : "ghost"}" onclick="syncBusinessNow()" ${online ? "" : "disabled"}>Sincronizar ahora</button>` : ""}
    </div>
  </div>`;
}
...
function renderAdminQuickLinks() {
  ...
  add("users", "Usuarios", "Roles y permisos");
...
function renderMore() {
  ...
  if (role === "owner") cards += card("users", "Usuarios", "Roles y permisos") + card("settings", "Configuración", "Administración del negocio");

  const roleText = role === "owner" ? "Dueño" : role === "admin" ? "Administrador" : "Empleado";
  const email = state.settings.email ? ` · ${esc(state.settings.email)}` : "";
  const accountPanel = `<div class="panel" style="margin-top:14px"><strong>Cuenta</strong><p class="muted">${roleText}${email}</p><div class="toolbar">${state.settings.pinEnabled && state.settings.ownerPin ? `<button class="btn ghost" onclick="lockApp()">Bloquear</button>` : ""}<button class="btn danger" onclick="logoutOwner()">Cerrar sesión</button></div></div>`;
