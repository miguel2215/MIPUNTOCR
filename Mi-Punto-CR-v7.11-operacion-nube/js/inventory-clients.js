...
window.saveProduct = async id => {
  ...
  let p=id?currentProducts().find(x=>x.id===id):null; if(!p){p={id:(crypto?.randomUUID?crypto.randomUUID():uid("p")),businessType:type()};state.products.push(p);} p.businessType=type();p.name=name;p.price=price;p.category=$("#pCategory").value.trim()||"Otros";p.stock=Number($("#pStock").value||0);p.variants=isProducts()?$("#pVariants").value.split(",").map(x=>x.trim()).filter(Boolean):[];
...
window.saveClient = async selectAfter => { const name = $("#cName").value.trim(); if (!name) return toast("Escribe el nombre."); const c = { id: (crypto?.randomUUID?crypto.randomUUID():uid("c")), name, phone: $("#cPhone").value.trim(), balance: 0 }; try{await cloudSaveClientFromApp(c);}catch(error){console.error(error);c.cloudPending=true;} await put("clients", c); state.clients.push(c); if (selectAfter) saleMeta.clientId = c.id; closeModal(); if (selectAfter) rerenderSale(); else renderClients(); toast(c.cloudPending?"Cliente guardado. Pendiente de nube.":"Cliente guardado en la nube."); };
