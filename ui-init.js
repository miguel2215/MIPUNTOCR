...
window.unlock = () => { const p = $("#pin").value.trim(); if (state.settings.ownerPin && p === state.settings.ownerPin) { role = ["owner","admin","employee"].includes(state.settings.cloudRole) ? state.settings.cloudRole : role; locked = false; lastActivityAt = Date.now(); return render(); } toast("PIN incorrecto."); };
