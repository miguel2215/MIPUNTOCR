...
function renderOnboarding() {
  ...
  if (authStep === "welcome") {
    $("#app").innerHTML = `<section class="onboarding"><div class="onboard-card simple-access">${brandHorizontal()}<h2>Tu negocio, más simple.</h2><p>Empieza a vender en segundos.</p><div class="choice-grid"><button class="btn primary full access-main" onclick="startGuest()">Empezar ahora</button><button class="btn ghost full" onclick="existingAccount()">Entrar</button><button class="btn ghost full" onclick="startAccount()">Crear cuenta</button><button class="btn ghost full" onclick="startEmployeeAccess()">Acceso de empleado</button></div><p class="setup-note" style="margin-top:14px">Puedes empezar sin cuenta. Cuando quieras, podrás guardar tu negocio en la nube.</p></div></section>`;
    return;
  }

  if (authStep === "guest") {
    $("#app").innerHTML = `<section class="onboarding"><div class="onboard-card"><button class="back-link" onclick="authBack('welcome')">‹ Volver</button>${brandHorizontal("brand-logo-horizontal compact")}<h2>Empecemos</h2><p>Solo necesitamos dos datos.</p><div class="field"><label>Nombre del negocio</label><input id="guestBusiness" maxlength="80" autofocus></div><div class="field"><label>Tipo de negocio</label><div class="choice-grid compact-choice"><button class="type-choice" onclick="finishGuest('food')"><strong>Comida</strong><small>Soda, restaurante o repostería</small></button><button class="type-choice" onclick="finishGuest('products')"><strong>Artículos</strong><small>Tienda, ropa, cosméticos y más</small></button><button class="type-choice" onclick="finishGuest('services')"><strong>Servicios</strong><small>Belleza, reparaciones y trabajos</small></button></div></div></div></section>`;
    return;
  }

  if (authStep === "employeeCode") {
    $("#app").innerHTML = `<section class="onboarding"><div class="onboard-card employee-access"><button class="back-link" onclick="authBack('welcome')">‹ Volver</button>${brandHorizontal("brand-logo-horizontal compact")}<h2>Acceso de empleado</h2><p>Ingresa tu código.</p><div class="employee-code-box"><span>CR</span><strong id="employeeCodeDigits">____</strong></div>${numericKeypadHtml('employeeCodeKey')}</div></section>`;
    window._employeeDigits = '';
    return;
  }
...
window.startGuest = () => { authStep = "guest"; renderOnboarding(); };
window.finishGuest = async businessType => {
  const businessName = $("#guestBusiness")?.value.trim();
  if (!businessName) return toast("Escribe el nombre del negocio.");
  state.settings = { ...state.settings, businessName, ownerName: "Dueño", businessType, businessId: state.settings.businessId || friendlyBusinessCode(), accountCreated: false, onboardingComplete: true, sessionActive: true, cloudLinked: false, cloudRole: "owner", guestMode: true, firstSaleCelebrated: false };
  await put("settings", state.settings);
  role = "owner"; authStep = "welcome"; screen = "home"; render();
  toast("Listo. Ya puedes empezar a vender.");
};
window.startEmployeeAccess = () => { authStep = "employeeCode"; renderOnboarding(); };
function numericKeypadHtml(handler) {
  return `<div class="numeric-keypad">${[1,2,3,4,5,6,7,8,9].map(n=>`<button onclick="${handler}('${n}')">${n}</button>`).join('')}<button class="key-muted" onclick="${handler}('back')">⌫</button><button onclick="${handler}('0')">0</button><button class="key-muted" onclick="${handler}('clear')">×</button></div>`;
}
window.employeeCodeKey = key => {
  let d = window._employeeDigits || '';
  if (key === 'back') d = d.slice(0,-1); else if (key === 'clear') d = ''; else if (/^\d$/.test(key) && d.length < 6) d += key;
  window._employeeDigits = d;
  const el = $("#employeeCodeDigits"); if (el) el.textContent = (d + '______').slice(0, Math.max(4,d.length)).replace(/_/g,'_');
  if (d.length >= 4) {
    // El backend CR1000+ se activa en la segunda parte de v7.17.
    const local = (state.settings.localEmployees || []).find(e => String(e.codeNumber) === d);
    if (local) { role='employee'; state.settings.memberDisplayName=local.name; state.settings.cloudPermissions=local.permissions||{}; authStep='welcome'; screen='home'; render(); toast(`Hola, ${local.name}.`); }
  }
};
window.startAccount = () => {
  setupDraft = {};
