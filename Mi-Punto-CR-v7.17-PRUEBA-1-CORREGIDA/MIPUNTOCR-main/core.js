...
function autoLockEnabled() {
  return !!(state.settings.onboardingComplete && state.settings.sessionActive !== false && state.settings.pinEnabled && state.settings.ownerPin && !locked);
}
...
function desktopNavItems() {
  ...
  if (role === "owner") items.push(["users", "Usuarios"], ["settings", "Configuración"]);
...
function render() {
  ...
  if (state.settings.sessionActive === false) { clearInactivityTimer(); authStep = "welcome"; return renderOnboarding(); }
